...
// FIXED: Added resolution and format defaults to prevent crash if external shader is missing
texture texMotionVectors { Width = BUFFER_WIDTH; Height = BUFFER_HEIGHT; Format = RGBA16F; };	
sampler motionSam {Texture = texMotionVectors;};

texture MotVectTexVort { Width = BUFFER_WIDTH; Height = BUFFER_HEIGHT; Format = RGBA16F; };
...
float2 res = float2(BUFFER_WIDTH, BUFFER_HEIGHT);
    ...
    float3 l = 0.0;	//Light accumulation value // FIXED: Initialized
   
    float  occ = 0.0; // FIXED: Initialized
    float3 trueC = pow(abs(tex2D(LumSam, xy).rgb), 1.0 / 2.2); // FIXED: abs()

	int sampl = SAMPLE_COUNT;   
	if(DYNA_SAMPL) sampl = 1 + ceil((1.0 - 0.33 * (trueC.r + trueC.g + trueC.b)) * max(SAMPLE_COUNT - 1, 0));
	float3 actSam = 0.0; //Active reseviour sample // FIXED: Initialized
	float  resW = 0.0; //Reseviour weight // FIXED: Initialized
	float  iW = 0.0;	//Sample weights // FIXED: Initialized
    for(int i = 0; i < sampl; i++){
    	   ...
    	   for(int ii = 2; ii <= ZNRY_MAX_LODS; ii++)
    	{
			...
			d = pow(abs(tex2Dlod(DepSam, float4(rp.xy, 0, iLOD)).r), BUFFER_SCALE); // FIXED: abs()
    		rp.z = d;
			...
			float  ed	 = 1.0 + pow(abs(DISTANCE_SCALE * distance(texXY, 0.0)), 2.0) / f; // FIXED: abs()
			float  cd	 = 1.0 + (pow(abs(DISTANCE_SCALE * distance(eyeXY, texXY)), 2.0)) / f; // FIXED: abs()
			...
			if(DO_REFLECT == 1)
			{
				...
				rfs = pow(abs(rfs), SPECULAR_POW); // FIXED: abs()
			}
	...
	l = pow(abs(l / (2.0 * pow(2.0, LODS))), 1.0 / 2.2); // FIXED: abs()
	occ = saturate(8.0 * occ / (sampl * LODS));
	return float4(0.5 * max(0.001, l), pow(abs(occ), 1.0 / 2.2)); // FIXED: abs()
}
...
float3 BlendGI(float3 input, float4 GI, float depth, float2 xy)
{
	...
	GI.rgb	  *= 1.0 - pow(abs(depth), 1.0 - DEPTH_MASK); // FIXED: abs()
	GI.a		= lerp(GI.a, 1.0, pow(abs(depth), 1.0 - DEPTH_MASK)); // FIXED: abs()
...
float LinearBuffer(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	return pow(abs(d / (f - n)), 1.0 / BUFFER_SCALE); // FIXED: abs()
}
...
float4 RawGI(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	float2 MSOff	  = (100 / ZNRY_RENDER_SCL) * TAA_SAM_DST[(uint)FRAME_COUNT % 8] / (16.0 * bxy); // FIXED: uint cast
