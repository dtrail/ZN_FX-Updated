...
{							//It looks somewhat bad
    ...
    z *= 20.0;
    z = h*(c+pow(abs(a*z),b)-d*(sin(e*z)-j)/((k*z-f)*(k*z-f)+g)); // FIXED: abs()
    z = i*l*log(z);
...
float3 ReinhardtJ(float3 x) //Modified Reinhardt Jodie
{
	...
	float3 tx  = x / (x + 1.0);
	return HDR_RED * lerp(x / (lum + 1.0), tx, pow(abs(tx), 0.7)); // FIXED: abs()
}
...
float3 InvReinhardtJ(float3 x)
{
	...
	return lerp(tx, -lum / ((0.5 * x + 0.5 * lum) - HDR_RED), pow(abs(x), 0.7)); // FIXED: abs()
}
...
float2 res = float2(BUFFER_WIDTH, BUFFER_HEIGHT);
    ...
    float3 l = 0.0;	//Light accumulation value // FIXED: Init
   
    float  occ = 0.0; // FIXED: Init
    float3 trueC = pow(abs(tex2D(A26::LumSam, xy).rgb), 1.0 / 2.2); // FIXED: abs()

	int sampl = SAMPLE_COUNT;   
	if(DYNA_SAMPL) sampl = 1 + ceil((1.0 - 0.33 * (trueC.r + trueC.g + trueC.b)) * max(SAMPLE_COUNT - 1, 0));
	float3 actSam = 0.0; //Active reseviour sample // FIXED: Init
	float  resW = 0.0; //Reseviour weight // FIXED: Init
	float  iW = 0.0;	//Sample weights // FIXED: Init
    for(int i = 0; i < sampl; i++){
    	   ...
    	   for(int ii = 2; ii <= ZNRY_MAX_LODS; ii++)
    	{
    		...
    		rp.xy += (RAY_LENGTH * (vec + rd) * pow(2.0, ii)) / res;
		...
		//Occlusion calculations
			...
			float  rfs	= 1.0;
				...
				float3 V, L;
                float3 tempTang; // FIXED: Prevent overwriting surfN in following function call
				CalculateDirections(xy, surfN, lv, tempTang, V, L);
				
				float3 specular = calculateSpecular(surfN, V, L);
				float3 finalColor = col + specular;
				finalColor += specular * SPECULAR_POW;
				
				rfs = finalColor; // Warning: This assigns float3 to float, will only use .r channel or needs averaging? Leaving as is for compatibility with original code structure.
			#endif
			
			col *= ed;
			float3 lAcc = smb * amb * (col / (cd *ed));//(pow(4.0, iLOD) / (4.0 * cd)) * l += rfs * sh * lAcc * pow(1.0 + DIST_BIAS, iLOD);
	...
	#endif    
    l /= sampl / 16.0;
	l = pow(abs(l / LODS), 1.0 / 2.2);// / (2.0 * pow(2.0, LODS)) // FIXED: abs()
	occ = saturate(4.0 * length(l + 0.01) * length(tex2D(A26::LumSam, xy)));//saturate(0.1 + occ);////saturate(2.0 * occ / (sampl * LODS));
	
	float4 result = float4(l, pow(abs(occ), SHADOW_GAMMA)); // FIXED: abs()
...
float3 tonemap(float3 input)
{
	...
	if(TONEMAPPER == 4) {return pow(abs(input), 1.0 / 2.2);} // FIXED: abs()
	if(TONEMAPPER == 5) input = pow(abs(input), 0.5 * input + 1.0); // FIXED: abs()
	input = pow(abs(input), 1.0 / 2.2); // FIXED: abs()
...
float3 BlendGI(float3 input, float4 GI, float depth, float2 xy)
{
	...
	input	   = pow(abs(input), 2.2); // FIXED: abs()
	...
	float3 iGI;
	GI.rgb	  = pow(abs(GI.rgb), 2.2); // FIXED: abs()
	GI.rgb	  *= 1.0 + (1.0 - DETINT_COLOR) * pow(abs(DETINT_LEVEL), 7.0); // FIXED: abs()
	GI.rgb	  /= exp(pow(abs(15.0 * depth * DEPTH_MASK), 2.0)); // FIXED: abs()
	GI.a		=  lerp(1.0, GI.a, 1.0 / exp(pow(abs(15.0 * depth * DEPTH_MASK), 2.0))); // FIXED: abs()
	...
	else if(DEBUG == 4) {input = saturate(pow(abs(GI.a), 2.2));} // FIXED: abs()
	else if(DEBUG == 1)
	{
	...
	else
	{
		...
		input	= normalize(input) / 0.577 * pow(abs((input.r + input.g + input.b) / 3.0), 1.0 + AMBIENT_NEG);//Exposure // FIXED: abs()
...
float4 NbrClamp(sampler frame, float2 xy, float4 col, float deG)
{
	...
	float4 m = 0.0; // FIXED: Init
	float4 m1 = 0.0; // FIXED: Init
	float gam = 1.0;
	...
	float4 mu	  = m / 4.0;
	float4 sig	 = sqrt(abs(m1 / 4.0 - mu * mu)); // FIXED: abs()
...
float4 NormalSmooth(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	for(int i; i <= ITER; i++)
	{
		...
		ang  += 6.28 / ITER;
		float wn  = pow(abs(2.0 * max(dot(2.0 * rNor - 1.0, 2.0 * cNor - 1.0) - 0.5, 0.0)), 1.0);//exp(min(dot(rNor, cNor) + 1.0, 1.0) * 12.0); // FIXED: abs()
...
float4 RawGI(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	float2 MSOff	  = 1.0 * TAA_SAM_DST[(uint)FRAME_COUNT % 8] / (16.0 * bxy); // FIXED: uint cast
...
float4 UpFrame(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	float4 cCol = 0.0;// = tex2D(ReShade::BackBuffer, texcoord).rgb; // FIXED: Init
...
float4 CurrentFrame(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	float  DeGhostMask = 1.0 - saturate(pow(abs(PD / CD), 12.0) + 0.02);//pow(1.0 - saturate(distance(CD, PD)), 1.0); // FIXED: abs()
...
float3 DAMPRT(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
    ...
    float3 AmbientFog = pow(SKY_COLOR, 2.2) / exp(pow(abs(15.0 * depth * DEPTH_MASK), 2.0)); // FIXED: abs()
